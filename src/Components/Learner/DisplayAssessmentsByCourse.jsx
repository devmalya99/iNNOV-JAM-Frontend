...
const DisplayAssessmentsByCourse = () => {
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen p-6 bg-gray-50 dark:bg-gray-900">
        ...
        <p className="text-gray-600 dark:text-gray-300 animate-pulse">Loading assessments...</p>
      </div>
    );
  }
  ...
  if (isError) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen p-6 bg-gray-50 dark:bg-gray-900">
        ...
        <p className="text-gray-600 dark:text-gray-300">No Assessment Created yet for this course </p>
        <button 
          onClick={() => refetch?.()} 
  ...
  const getAssessmentIcon = (type) => {
    switch (type?.toLowerCase()) {
      ...
      default:
        return <FaClipboardCheck className="text-blue-500 dark:text-blue-400" />;
    }
  };
              ...
              >
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center space-x-3">
                    {getAssessmentIcon(assessment?.assessmentId?.assessment_type)}
                    <span className="text-lg font-semibold text-gray-900 dark:text-white">
                      {assessment?.assessment_type?.replace(/_/g, ' ').charAt(0).toUpperCase() + 
                       assessment?.assessment_type?.replace(/_/g, ' ').slice(1)}
                    </span>
                  </div>
