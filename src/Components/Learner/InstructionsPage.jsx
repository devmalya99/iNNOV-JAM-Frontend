...
import { FaSpinner, FaExclamationCircle, FaArrowLeft, FaCheckCircle, FaClock, FaFileAlt } from 'react-icons/fa';

const InstructionsPage = () => {
  const navigate = useNavigate();
  const { assessmentId } = useParams();
  const { data, refetch, error, isLoading, isError } = FetchAssessmentInstructions(assessmentId);
    ...
    return (
      <div className="fixed inset-0 bg-gray-900/50 backdrop-blur-sm flex items-center justify-center">
        <div className="bg-white dark:bg-gray-800 rounded-xl p-8 text-center">
          ...
          <p className="text-gray-600 dark:text-gray-300">Loading instructions...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="fixed inset-0 bg-gray-900/50 backdrop-blur-sm flex items-center justify-center">
        <div className="bg-white dark:bg-gray-800 rounded-xl p-8 text-center max-w-md">
          ...
          <h2 className="text-xl font-semibold text-gray-900 dark:text-gray-100 mb-2">Error Loading Instructions</h2>
          <p className="text-gray-600 dark:text-gray-300 mb-4">{error?.message || 'Failed to load assessment instructions'}</p>
          <button 
            onClick={() => refetch()} 
      ...
      <div className="bg-white dark:bg-gray-800 shadow-xl rounded-xl w-full max-w-2xl overflow-hidden">
        ...
        <div className="p-6 border-b border-gray-200 dark:border-gray-700">
          ...
          <div className="flex items-center text-gray-600 dark:text-gray-300 text-sm">
            ...
            <span className="capitalize">{data?.assessment_type?.replace(/_/g, ' ') || 'Assessment'}</span>
          </div>
        </div>
        ...
        <div className="px-6 py-4 bg-gray-50 dark:bg-gray-700/50 flex justify-between items-center">
          ...
          <div className="space-x-4">
            ...
            <button
              onClick={() => navigate(`/home/learner/answer-writing/${assessmentId}`)}
